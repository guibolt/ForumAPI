...
namespace ApiForum.Controllers
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        public PostsController(IMapper mapper, ForumContext contexto)
        {
            _mapper = mapper;
            _contexto = contexto;
        }
        ...
        {
            var Core = new PostCore(publicacao, _mapper, _contexto).CadastrarPost(tokenAutor);
        ...
        {
            var Core = new PostCore(_contexto).ListarPorId(id, tokenAutor);
            return Core.Status ? Ok(Core) : (IActionResult)BadRequest(Core);
        }
        ...
        {
            var Core = new PostCore(_contexto).ListarTodos(tokenAutor);
            return Core.Status ? Ok(Core) : (IActionResult)BadRequest(Core);
        }
        ...
        {
            var Core = new PostCore(_contexto).DeletarPost(tokenAutor,id);
            return Core.Status ? Ok(Core) : (IActionResult)BadRequest(Core);
        }
        //Chamando o metodo de listar todos da core 
        [HttpPut("{id}")]
        ...
        {
            var Core = new PostCore(_mapper, _contexto).EditarPost(id,postAtt, tokenAutor);
            return Core.Status ? Ok(Core) : (IActionResult)BadRequest(Core);
        }
        //Chamando o metodo de cadastro da core 
        //[HttpPost("Votar")]
        ...
        //{
        //    var Core = new PostCore(_mapper, _contexto).VotarPost(tokenAutor, Voto);
        //    return Core.Status ? Ok(Core) : (IActionResult)BadRequest(Core);
        //}
    }
}                                                    